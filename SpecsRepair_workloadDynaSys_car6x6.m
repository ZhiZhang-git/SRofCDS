...
for i_plot=1:n_partition*n_partition
     ...
     plot(store_traj_positi(2*(i_plot-1)+1,:),store_traj_positi(2*i_plot,:));
...
for i_store_circle=1:n_store_PoinTime
    ...
    plot(l1_chec_circl(i_store_circle,:),l2_chec_circl(i_store_circle,:),'','LineWidth', 2);
...
%% for inner
%axis([1.27 1.42 0.95 1.16]); % [xmin xmax ymin ymax]
%xticks(1.255:0.05:1.26); % X 轴刻度每隔 2 显示一次
%yticks(1.01:0.05:1.1);
