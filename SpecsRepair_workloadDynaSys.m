%Zhi Zhang, 09th. 10. 2024, Imperial College London,
...
n_partition=2;%which means that the initial region is partitioned into 2x2 type
...
figure(1)
plot(store_trajectory(1,:),store_trajectory(2,:),'','LineWidth', 3);
hold on 
plot(store_trajectory(3,:),store_trajectory(4,:),'','LineWidth', 3);
hold on
plot(store_trajectory(5,:),store_trajectory(6,:),'','LineWidth', 3);
hold on
plot(store_trajectory(7,:),store_trajectory(8,:),'','LineWidth', 3);
hold on 
...
for i_store_circle=1:n_store_PoinTime
    ...
    plot(l1_chec_circl(i_store_circle,:),l2_chec_circl(i_store_circle,:),'','LineWidth', 5);
...
% Control axis limits and aspect ratio
% axis([1.15 1.55 1.15 1.55]); % [xmin xmax ymin ymax]
axis square; % Optional: Makes the axes square
% xticks(1.15:0.125:1.55); % X 轴刻度每隔 2 显示一次
% yticks(1.15:0.125:1.55);
% for the second initial region
...
%axes('Position', [0.6 0.6 0.6 0.6]); % 定义嵌套图的位置和大小 [x y width height]
%plot(x, y, 'r');
%xlim([1.1 1.5]); % 设置嵌套图的显示区域
%ylim([1.1 1.5]);
